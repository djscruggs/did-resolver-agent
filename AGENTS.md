...
- **Use numbered step comments in multi-step validation pipelines.** Complex validation functions like checkDelegation use numbered step comments (// 0. Verify auth proof, // 1. Parse the VC, // 2. Check subject match, // 3. Resolve issuer DID, // 4. Verify signature, // 4.5. Check revocation, // 5. Check claims) to make the pipeline stages clear. Steps can use fractional numbers (4.5) when inserting new stages between existing ones to avoid renumbering.
  ```
  // Good
  ...
  // Bad
    // Verify everything in one block with no structure
    const payload = parseCredential(vcJwt);
    if (!payload) return fail;
    if (payload.sub !== agentDid) return fail;
    const resolution = await resolveDID(payload.iss);
    // ... continues without step markers ...
  ```
