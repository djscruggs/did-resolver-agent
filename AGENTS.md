...
- **Use Anthropic.MessageParam namespace type instead of direct sub-module imports.** When using the Anthropic SDK, prefer accessing types through the Anthropic namespace (e.g., Anthropic.MessageParam, Anthropic.Tool, Anthropic.ToolResultBlockParam) rather than importing types from deep sub-module paths like '@anthropic-ai/sdk/resources/messages.js'. The namespace approach is more stable across SDK versions and avoids import resolution errors.
