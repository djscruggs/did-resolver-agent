...
| `verify_delegation_chain` | Verify a chain of delegation VCs from root issuer to agent |
...
- **Separate pure logic from IO boundaries with dedicated lib/ vs tools/ layers and Tool-suffixed wrappers.** Core business logic (crypto, VC operations, chain validation, claim predicates) lives in \`lib/\` files that are pure and testable in isolation. IO-dependent code (DID resolution, network calls) lives in \`tools/\` wrappers that compose pure functions with IO. Tool wrapper functions are named with a 'Tool' suffix to make the IO boundary explicit at the call site. Every module starts with a JSDoc comment stating its purpose and IO status ('No IO — testable in isolation' for pure modules, 'IO boundary' for tool modules). This architecture enables testing core logic without mocks and makes dependencies immediately visible.

  **Good:**

  // lib/chain.ts — pure logic, annotated and testable without mocks
  /\*\*

  - Delegation chain verification.
  - Pure (keys passed in, no IO).
    \*/
    export async function verifyDelegationChain(
    links: ChainLink\[],
    leafAgentDid: string,
    maxDepth = 5
    ): Promise<{ valid: boolean; depth: number; reason?: string }> { ... }

  // tools/verifyChain.ts — IO boundary with Tool suffix
  /\*\*

  - IO boundary for delegation chain verification.
  - Resolves DIDs, then calls pure verifyDelegationChain.
    \*/
    export async function verifyDelegationChainTool(input: {
    vcChain: string\[];
    agentDid: string;
    }): Promise<...> {
    const chainLinks = await Promise.all(
    input.vcChain.map(jwt => resolveDIDFromJWT(jwt)) // IO
    );
    return verifyDelegationChain(chainLinks, input.agentDid); // pure
    }

  **Bad:**

  // Mixed concerns: IO and logic in one function, no boundary annotation
  export async function verifyDelegationChain(vcChain: string\[], agentDid: string) {
  for (const jwt of vcChain) {
  const resolution = await fetch(`https://resolver/${extractDID(jwt)}`); // hidden IO
  const verified = crypto.verify(resolution.publicKey, jwt); // logic mixed with IO
  // ... more inline verification logic ...
  }
  }

  // Ambiguous wrapper without Tool suffix — caller can't see IO boundary
  export async function verifyDelegation(vcChain: string\[], agentDid: string) { ... }
...
- **Define explicit TypeScript interfaces for all function inputs and outputs.** Every public function uses named interfaces for its input and output types rather than inline object types or \`any\`. Input interfaces are named after their purpose (e.g., \`CheckDelegationInput\`, \`IssueInput\`, \`ChainLink\`). Result types use discriminated patterns (e.g., \`CheckClaimsResult\` as a union of satisfied/unsatisfied). All interface fields include JSDoc descriptions for complex or non-obvious members.
  ...
  **Good:**
  ...
  /\*\* JWT-format VC */
  vcJwt: string;
  /*\* Claims that must satisfy predicates for authorization */
